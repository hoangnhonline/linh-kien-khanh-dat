...
class CateController extends Controller
{
    ...
    */
    ...
    {   
        ...
        if($rs){//danh muc cha
            ...
            $query = SanPham::where('loai_sp_id', $loai_sp_id)
            ...
            $hoverInfo = HoverInfo::where('loai_sp_id', $rs->id)->orderBy('display_order', 'asc')->orderBy('id', 'asc')->get();
   ...
   public function banChay(Request $request)
    {
        ...
        $cateArr = Cate::where('status', 1)->where('loai_sp_id', $loai_sp_id)->get();

        
        $query = SanPham::where('loai_sp_id', $loai_sp_id)->where('so_luong_ton', '>', 0)->where('price', '>', 0)
                ->leftJoin('sp_hinh', 'sp_hinh.id', '=','product.thumbnail_id')
                ->leftJoin('sp_thuoctinh', 'sp_thuoctinh.sp_id', '=','product.id')
                ->select('sp_hinh.image_url', 'product.*', 'thuoc_tinh')              
    ...
    public function giamGia(Request $request)
    {
        ...
        $productArr = SanPham::where('loai_sp_id', $loai_sp_id)->where('is_sale', 1)->where('so_luong_ton', '>', 0)->where('price', '>', 0)
                ->leftJoin('sp_hinh', 'sp_hinh.id', '=','product.thumbnail_id')
                ->leftJoin('sp_thuoctinh', 'sp_thuoctinh.sp_id', '=','product.id')
                ->select('sp_hinh.image_url', 'product.*', 'thuoc_tinh')
                //->where('sp_hinh.image_url', '<>', '')
    ...
    public function theoGia(Request $request)
    {
        ...
        $tmp = DB::table('price_range')->where('loai_sp_id', $loai_sp_id)->where('alias', $slugGia)->first();
        $title = $tmp->name;
        $from = $tmp->from;
        $to = $tmp->to;        
        $cateArr = Cate::where('status', 1)->where('loai_sp_id', $loai_sp_id)->get();
    ...
    */
    ...
    public function cate(Request $request)
    {
        ...
        $loai_sp_id = $rs->id;
        $rsCate = Cate::where(['loai_sp_id' => $loai_sp_id, 'slug' => $slug])->first();
        $cate_id = $rsCate->id;

        $cateArr = Cate::where('status', 1)->where('loai_sp_id', $loai_sp_id)->get();

        
        $query = SanPham::where('cate_id', $rsCate->id)->where('loai_sp_id', $loai_sp_id)->where('so_luong_ton', '>', 0)->where('price', '>', 0)
                ->leftJoin('sp_hinh', 'sp_hinh.id', '=','product.thumbnail_id')
                ->leftJoin('sp_thuoctinh', 'sp_thuoctinh.sp_id', '=','product.id')
                ->select('sp_hinh.image_url', 'product.*', 'thuoc_tinh');
                    if($rs->price_sort == 0){
